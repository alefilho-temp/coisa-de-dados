...
public class ProductDAOImpl implements ProductDAO {
    ...
    public void addProduct(ProductModel product) throws ClassNotFoundException, SQLException {
        String sql = "INSERT INTO Produto (nome, preco_Unitario, Imagem, descricao, VendedorCadastro_id, Categoriaid, quantidade, Desconto) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
        try (Connection con = DBConnection.getConnection(); PreparedStatement pst = con.prepareStatement(sql)) {
            ...
            pst.setInt(7, 10);
            pst.setInt(8, (int) product.getDiscount());
            pst.executeUpdate();
        }
    }
    ...
    public void updateProduct(ProductModel product) throws Exception {
        String sql = "UPDATE Produto SET nome=?, preco_Unitario=?, Imagem=?, descricao=?, Categoriaid=?, VendedorCadastro_id=?, Desconto=? WHERE Id_Produto=?";
        try (Connection con = DBConnection.getConnection(); PreparedStatement pst = con.prepareStatement(sql)) {
            pst.setString(1, product.getName());
            ...
            pst.setInt(5, product.getCategoryId());
            pst.setInt(6, product.getSellerId());
            pst.setInt(7, (int) product.getDiscount());
